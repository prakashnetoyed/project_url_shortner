...
const CreateShortUrl = async function (req, res) {
  try {
    ...
    if (Object.keys(body).length === 0) {
      return res
        .status(400)
        .send({ status: false, message: "Please enter the data" });
    }

    if (!body.longUrl) {
      return res
        .status(400)
        .send({ status: false, message: "Please enter the URL" });
    }

    if (!validUrl.isWebUri(body.longUrl)) {
      return res
        .status(400)
        .send({ status: false, message: "Looks like not a valid URL" });
    }
    ...
    const shortUrl = "43.204.199.206:3000" + "/" + urlCode;
